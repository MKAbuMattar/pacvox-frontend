...
export const PlaneContainer = styled.section`
  --dark: 1;
  --base-size: 20;
  ...
  *:before {
  ...
  display: grid;
  place-items: center;
  background-image: radial-gradient(circle, var(--bg), #171424);
  overflow: hidden;
  ...
  .cloud__shift {
    height: 100%;
    width: 100%;
    -webkit-animation: pan calc(var(--speed, 5) * 1s)
      calc(var(--delay, 5) * -1s) infinite ease-in-out both;
  ...
  .cloud__body {
    height: 100%;
    width: 100%;
    -webkit-animation: scale calc(var(--speed, 5) * 1s)
      calc(var(--delay, 5) * -1s) infinite linear;
